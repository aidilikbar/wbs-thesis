...
function badgeClasses(value: string) {
  ...
  if (["critical", "escalated", "danger", "closed"].includes(normalized)) {
    return "border border-[rgba(237,28,36,0.18)] bg-[rgba(237,28,36,0.12)] text-[var(--accent-strong)]";
  }

  if (["high", "investigation", "investigating", "warning"].includes(normalized)) {
    return "border border-[rgba(16,17,20,0.12)] bg-[rgba(16,17,20,0.06)] text-[var(--foreground)]";
  }

  if (["resolved", "active", "assessment", "submitted", "under_review"].includes(normalized)) {
