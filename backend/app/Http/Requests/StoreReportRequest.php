...
class StoreReportRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'category' => ['nullable', 'string', Rule::in(array_keys(config('wbs.categories')))],
            'description' => ['required', 'string', 'min:30', 'max:5000'],
            'reported_parties' => ['nullable', 'array', 'min:1', 'max:10'],
            'reported_parties.*.full_name' => ['required_with:reported_parties', 'string', 'max:255'],
            'reported_parties.*.position' => ['required_with:reported_parties', 'string', 'max:255'],
            'reported_parties.*.classification' => ['required_with:reported_parties', 'string', Rule::in(array_keys(config('wbs.reported_party_classifications')))],
            'incident_date' => ['nullable', 'date', 'before_or_equal:today'],
            'incident_location' => ['nullable', 'string', 'max:255'],
            'accused_party' => ['nullable', 'string', 'max:255'],
            'evidence_summary' => ['nullable', 'string', 'max:1500'],
            'confidentiality_level' => ['nullable', 'string', Rule::in(array_keys(config('wbs.confidentiality_levels')))],
